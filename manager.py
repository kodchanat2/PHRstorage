...
def save_batch(table, rowkey, batch_data):
	...
	#XXprint client.request_enforcement(rowkey[:rowkey.find('_')], 'save_batch', 'POST', rowkey)
...
def fetch(table, rowkey, *args):
	c = starbase.Connection(port=HBASE_PORT)
	#XX# print client.request_enforcement('admin', 'path/path', 'GET', '1840400078741_555') IT'S work!
	# c = starbase.Connection(host=HBASE_HOST, port=HBASE_PORT)
	table = c.table(table)
	if not args:
		result = table.fetch(
	...
	#XXprint client.request_enforcement(rowkey[:rowkey.find('_')], 'fetch', 'GET', rowkey)
	return result
...
def fetch_from(table, start_row, *args):
	c = starbase.Connection(port=HBASE_PORT)
	#XX# print client.request_enforcement(rowkey[:rowkey.find('_')], req_path_info, req_method, req_service)
	# c = starbase.Connection(host=HBASE_HOST, port=HBASE_PORT)
...
def delete_row(table, rowkey):
	c = starbase.Connection(port=HBASE_PORT)
	#XXprint client.request_enforcement(rowkey[:rowkey.find('_')], 'delete_row', 'GET', rowkey)
