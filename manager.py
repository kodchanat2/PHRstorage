...
def save_batch(table, rowkey, batch_data):
	...
	print client.request_enforcement(rowkey[:rowkey.find('_')].encode(), 'save_batch'.encode(), 'POST', rowkey)
...
def fetch(table, rowkey, *args):
	c = starbase.Connection(port=HBASE_PORT)
	print client.request_enforcement(rowkey[:rowkey.find('_')].encode(), 'fetch'.encode(), 'GET', rowkey)
	# c = starbase.Connection(host=HBASE_HOST, port=HBASE_PORT)
	table = c.table(table)
	if not args:
		return table.fetch(
...
def fetch_from(table, start_row, *args):
	c = starbase.Connection(port=HBASE_PORT)
	# print client.request_enforcement(rowkey[:rowkey.find('_')].encode(), req_path_info.encode(), req_method, req_service)
	# c = starbase.Connection(host=HBASE_HOST, port=HBASE_PORT)
...
def insert_data(table, rowkey, columfamily, columqualifier, value):
	c = starbase.Connection(port=HBASE_PORT)
	print client.request_enforcement(rowkey[:rowkey.find('_')].encode(), 'insert_data'.encode(), 'POST', rowkey)
	# c = starbase.Connection(host=HBASE_HOST, port=HBASE_PORT)
	table = c.table(table)
...
def delete_row(table, rowkey):
	c = starbase.Connection(port=HBASE_PORT)
	print client.request_enforcement(rowkey[:rowkey.find('_')].encode(), 'delete_row'.encode(), 'GET', rowkey)
